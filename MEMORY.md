...
- **React 19 StrictMode + WebSocket**: StrictMode mounts effects twice in dev. WsClient.destroy() sets `destroyed=true` and removes handlers before close. `onclose` always reconnects if `destroyed=false`. One WsClient per chart — connection effect has empty deps (creates once), subscription effect updates on param changes without recreating socket. `pendingSub` buffers subscribe until onopen.
