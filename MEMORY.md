...
- **React 19 StrictMode + WebSocket**: StrictMode mounts effects twice in dev. WsClient.disconnect() sets `isUnmounting=true` and removes handlers before close. `onclose` checks `isUnmounting` — if false (real server drop), schedules reconnect with exponential backoff (1s→2s→4s→max 10s). `cleanClose()` helper removes all handlers then closes to prevent "closed before established". Re-subscribe happens automatically on `onopen` from saved `subscriptions` array.
