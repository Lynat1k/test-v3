...
- **React 19 StrictMode + WebSocket**: StrictMode mounts effects twice in dev. WsClient.disconnect() must set `destroyed=true` flag and remove ALL event handlers (onopen/onmessage/onerror/onclose) BEFORE calling ws.close(). Otherwise onclose fires on the destroyed instance and calls onDisconnect, interfering with the new instance. The `destroyed` flag prevents handlers from firing after cleanup.
